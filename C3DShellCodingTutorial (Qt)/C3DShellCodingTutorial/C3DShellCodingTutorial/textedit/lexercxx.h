...
//! \brief The LexerCxx class encapsulates the Scintilla C++
//! lexer.
class LexerCxx : public QsciLexer
{
    Q_OBJECT

private:
    // Строка с классами c3d
    std::string m_c3dClasses;
    // Строка с функциями c3d
    std::string m_c3dFunctions;
    // Строка с ключевыми слова языка C++
    std::string m_keyWordsCpp;

public:
    void setClasses (const std::string classNames);

    void setFunctions(const std::string functionNames);
    ...
    LexerCxx(QObject *parent = 0, bool caseInsensitiveKeywords = false);
...
private:
    ...
    LexerCxx(const LexerCxx &);
    LexerCxx &operator=(const LexerCxx &);
};